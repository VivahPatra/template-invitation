...
import { WeddingConfig, WeddingEvent, StoryMilestone, Blessing, FamilyMember, InfoCard, Accommodation } from '@/types/wedding.types'
...
interface EditModeContextType {
  ...
  updateBlessing: (index: number, field: string, value: string) => void
  ...
  updateAccommodation: (index: number, field: string, value: string) => void
}
...
export function EditModeProvider({ children }: { children: ReactNode }) {
  ...
  const updateInfoCard = useCallback((index: number, field: string, value: string) => {
    setData(prev => {
      ...
      infoCards[index] = { ...infoCards[index], [field]: value } as InfoCard
  ...
  const updateBlessing = useCallback((index: number, field: string, value: string) => {
    setData(prev => {
      const blessings = [...prev.blessings]
      blessings[index] = { ...blessings[index], [field]: value } as Blessing
      const next = { ...prev, blessings }
      sendToParent(next)
      return next
    })
  }, [])
  ...
  const updateAccommodation = useCallback((index: number, field: string, value: string) => {
    setData(prev => {
      const accommodations = [...prev.venue.accommodations]
      accommodations[index] = { ...accommodations[index], [field]: value } as Accommodation
      const next = { ...prev, venue: { ...prev.venue, accommodations } }
      sendToParent(next)
      return next
    })
  }, [])
  ...
  const updateNestedField = useCallback((arrayField: string, index: number, field: string, value: string) => {
    switch (arrayField) {
      ...
      case 'blessings': return updateBlessing(index, field, value)
      case 'familyBride': return updateFamilyBride(index, field, value)
      case 'familyGroom': return updateFamilyGroom(index, field, value)
      case 'accommodations': return updateAccommodation(index, field, value)
      default: break
    }
  }, [updateEvent, updateStory, updateInfoCard, updateBlessing, updateFamilyBride, updateFamilyGroom, updateAccommodation])

  return (
    <EditModeContext.Provider value={{ isEditing, data, updateField, updateNestedField, updateEvent, updateStory, updateInfoCard, updateBlessing, updateFamilyBride, updateFamilyGroom, updateAccommodation }}>
      {children}
